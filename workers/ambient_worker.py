...
def process(job):
 job_id=job['id']; payload=job.get('payload') or {}; image_url=validate_image_url(payload.get('image_url','')); duration_hours=float(payload.get('duration_hours',0.0167)); title=str(payload.get('title') or 'Ambient World'); label=str(payload.get('thumbnail_label') or 'SOCIALIZED AMBIENT WORLDS')
