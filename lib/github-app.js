...
async function dispatchRenderRequested(jobId) {
  const owner = process.env.GITHUB_REPO_OWNER || 'jbnikky13';
  const repo = process.env.GITHUB_REPO_NAME || 'Socialized';
  if (!jobId) throw new Error('jobId is required for render dispatch');
  ...
  const response = await fetch(`https://api.github.com/repos/${owner}/${repo}/dispatches`, {
    ...
    body: JSON.stringify({
      ...
      client_payload: { source: 'socialized-vercel', job_id: String(jobId) },
    }),
  });
  const text = await response.text();
  if (!response.ok) {
  ...
  return { dispatched: true, event_type: 'render_requested', job_id: String(jobId) };
}
