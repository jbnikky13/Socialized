...
from services.minimax_h3 import api_key, build_episode, image_data_url, estimate_cost
...
st.caption("Campaign/story → screenplay → MiniMax H3 → finished MP4 → Supabase → YouTube")

campaign = st.session_state.get("last_campaign") or {}
reused = st.session_state.get("reused_content", [])
default_script = campaign.get("script", "") or next((x.get("body", "") for x in reused if x.get("platform") == "youtube" and x.get("content_type") == "video"), "")
...
with st.sidebar:
    ...
    if api_key(): st.success("MiniMax API key detected")
    else: st.warning("MINIMAX_API_KEY missing")
    st.info("Cost is estimated before generation. H3 is billed per output second.")

st.subheader("1. Episode")
title = st.text_input("Episode title", value=default_title)
story = st.text_area("Story / script", value=default_script, height=180, placeholder="Use your saved campaign script or enter the story here.")
...
for i in range(int(char_count)):
    with cols[i % len(cols)]:
        ...
        if upload: st.image(upload, width=150)
...
if not story.strip(): st.info("You can use the saved YouTube script above or enter a new story.")

estimated = estimate_cost(scenes, resolution=resolution)
seconds = sum(s["duration"] for s in scenes)
rate = 0.13 if resolution == "2K" else 0.08
st.metric("Estimated generation cost", f"${estimated:.2f}", f"{seconds}s × ${rate:.2f}/sec")
st.caption("Estimate covers video generation only. Actual billing can vary with provider pricing, retries, or future model changes.")
if estimated > 10: st.warning("This episode is estimated above $10. Consider 768P or fewer/shorter scenes before generating.")

if st.button("🚀 Generate Photoreal Episode", type="primary", use_container_width=True, disabled=(not api_key() or bool(missing))):
    run_dir = Path(tempfile.mkdtemp(prefix="socialized_h3_")); progress_box = st.empty(); progress = st.progress(0)
    try:
        ...
        progress.progress(100); progress_box.success(f"Episode generated: {len(task_ids)} scenes")
        video_data = final_path.read_bytes(); st.video(video_data)
        ...
        else: st.info("No saved campaign is active, so the MP4 was not attached to Supabase.")
        st.session_state["microdrama_task_ids"] = task_ids; st.session_state["microdrama_title"] = title
    except Exception as exc:
        message = str(exc)
        if "MINIMAX_INSUFFICIENT_BALANCE" in message:
            st.error("💳 MiniMax balance is insufficient. Your API connection is working, but the MiniMax account/team needs more video credits.")
            st.warning(f"Estimated cost: ${estimated:.2f} for {seconds} seconds at {resolution}. Add credits to MiniMax and retry. The API rejected the request before creating a video task.")
        else:
