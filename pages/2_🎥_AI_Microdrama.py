...
story = st.text_area("Campaign script / story", value=default_script, height=160, placeholder="Use the saved campaign script or paste a story.")

st.subheader("2. Recurring characters")
count = st.number_input("Characters", 1, 6, 2, 1)
characters=[]; cols=st.columns(min(3,int(count)))
...
for i in range(int(scene_count)):
    with st.expander(f"Scene {i+1}", expanded=i==0):
        ...
        duration=st.slider("Duration",4,15,5,key=f"vp_duration_{i}")
        prompt=f"Photorealistic live-action microdrama. Natural human skin, realistic eyes, hair, hands and body movement. Cinematic believable lighting. Preserve recurring character identity from reference images. No cartoon, illustration, warped faces, extra fingers, text overlays or baked-in subtitles. SETTING: {setting}. CHARACTERS: {', '.join(selected)}. DETAILS: {'; '.join(c['name']+': '+c['description'] for c in characters if c['name'] in selected and c['description'])}. ACTION/CAMERA: {action}. Exact spoken dialogue: {dialogue or '[No spoken dialogue]'}. Speak the supplied dialogue naturally and synchronize mouth movement and facial expression to the words; do not paraphrase or invent dialogue."
...
if st.button("🚀 Generate Episode",type="primary",use_container_width=True,disabled=(not available(provider) or bool(missing))):
    work=Path(tempfile.mkdtemp(prefix="socialized_video_")); clips=[]; progress=st.progress(0); status=st.empty()
    try:
