...
def validate_media(path: str, max_mb: int = 512) -> tuple[bool, str]:
    if not path: return False, "No media file supplied."
    if not os.path.exists(path): return False, "Media file does not exist."
    size_mb = os.path.getsize(path) / (1024 * 1024)
    if size_mb > max_mb: return False, f"Media exceeds the {max_mb} MB limit."
...
def upload_asset(path: str, campaign_id: str, content_id: str | None = None, asset_type: str = "video") -> dict:
    p = Path(path)
    if not p.exists(): raise FileNotFoundError(f"Media file not found: {p}")
    mime = mimetypes.guess_type(p.name)[0] or "application/octet-stream"
    storage_path = f"campaigns/{campaign_id}/{asset_type}/{p.name}"
    sb = client()
    with p.open("rb") as fh:
        sb.storage.from_(BUCKET).upload(storage_path, fh, {"content-type": mime, "upsert": "true"})
    public_url = sb.storage.from_(BUCKET).get_public_url(storage_path)
    return sb.table("media_assets").insert({"campaign_id": campaign_id, "content_id": content_id, "asset_type": asset_type, "storage_path": storage_path, "public_url": public_url, "mime_type": mime}).execute().data[0]
