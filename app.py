...
st.title("📣 Socialized")
st.caption("One idea → AI campaign → X + YouTube → approval → publish → learn")
x_service = XService()
...
with st.sidebar:
    ...
    if st.button("🔗 Connect / Check YouTube", use_container_width=True):
        try:
            ...
            else: st.warning("No YouTube channel returned.")
        except Exception as e: st.error(str(e))
    st.success("X API configured") if x_service.configured() else st.warning("X API not configured")

research_tab, create_tab, queue_tab, youtube_tab, x_tab, analytics_tab = st.tabs(["🔎 Research", "✍️ Create", "📅 Campaigns", "▶️ YouTube", "𝕏 X", "📊 Analytics"])
...
with youtube_tab:
    ...
    st.write("Connect your YouTube account from the sidebar, then upload an approved video.")
    video_path = st.text_input("Video file path", value="")
    yt_title = st.text_input("Video title", value=st.session_state.get("last_campaign", {}).get("name", ""))
    yt_description = st.text_area("Description", height=140)
    yt_tags = st.text_input("Tags, comma separated")
    privacy = st.selectbox("Visibility", ["private", "unlisted", "public"])
    if st.button("📤 Upload to YouTube", type="primary"):
        if approval_required and not st.checkbox("I approve this video for publishing", key="yt_approval"): st.warning("Approval required before publishing.")
        elif not video_path or not os.path.exists(video_path): st.error("Video file path is missing or the file does not exist on the server.")
        else:
            try:
                video_id = upload_video(get_service(), video_path, yt_title, yt_description, [x.strip() for x in yt_tags.split(",") if x.strip()], privacy=privacy)
                st.success(f"Uploaded to YouTube. Video ID: {video_id}")
            except Exception as ex: st.error(str(ex))

with x_tab:
    st.subheader("𝕏 Publisher")
    x_text = st.text_area("Post", max_chars=280, height=100); st.caption(f"{len(x_text)}/280 characters")
    ...
    st.divider(); st.subheader("Thread publisher")
...
with analytics_tab:
    ...
    if st.button("Refresh YouTube stats"):
        try:
            info = channel_info(get_service())
            if info:
                stats = info.get("statistics", {}); a,b,c = st.columns(3)
                a.metric("Subscribers", stats.get("subscriberCount", "Hidden")); b.metric("Views", stats.get("viewCount", "0")); c.metric("Videos", stats.get("videoCount", "0"))
        except Exception as ex: st.error(str(ex))
