...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const isImageDelete = String(req.url || '').includes('/api/delete-image') || String(body.action || '').toLowerCase() === 'image';
    ...
    for (const url of urls) {
      try {
        ...
        if (i >= 0) paths.push(decodeURIComponent(u.pathname.slice(i + marker.length)));
      } catch {}
    }
    ...
    return res.status(500).json({ error: e.message || 'Could not complete delete request' });
  }
};
